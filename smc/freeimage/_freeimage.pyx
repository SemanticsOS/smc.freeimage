...
cdef extern from "FreeImage.h":
    # WITH GIL!
    ctypedef void (*FreeImage_OutputMessageFunction)(FREE_IMAGE_FORMAT, const_char_ptr )
    cdef void FreeImage_SetOutputMessage(void (*)(FREE_IMAGE_FORMAT, const_char_ptr ))

cdef extern from "smc_fi.h" nogil:
    ctypedef FIBITMAP* (*FI_ConvertFunction)(FIBITMAP *dib)
    int FREEIMAGE_TURBO
...
_META_MODELS = {
    "FIMD_NODATA" : FIMD_NODATA,
    "FIMD_COMMENTS" : FIMD_COMMENTS,
    "FIMD_EXIF_MAIN" : FIMD_EXIF_MAIN,
    "FIMD_EXIF_EXIF" : FIMD_EXIF_EXIF,
    "FIMD_EXIF_GPS" : FIMD_EXIF_GPS,
    "FIMD_EXIF_MAKERNOTE" : FIMD_EXIF_MAKERNOTE,
    "FIMD_EXIF_INTEROP" : FIMD_EXIF_INTEROP,
    "FIMD_IPTC" : FIMD_IPTC,
    "FIMD_XMP" : FIMD_XMP,
    "FIMD_GEOTIFF" : FIMD_GEOTIFF,
    "FIMD_ANIMATION" : FIMD_ANIMATION,
    "FIMD_CUSTOM" : FIMD_CUSTOM,
    }

_COLOR_TYPE_NAMES = {
    FIC_MINISWHITE : "White",
    FIC_MINISBLACK : "Black",
    FIC_RGB : "RGB",
    FIC_PALETTE : "Palette",
    FIC_RGBALPHA : "RGBA",
    FIC_CMYK : "CMYK"
...
cdef void setError(FREE_IMAGE_FORMAT format, const_char_ptr msg):
...
#        self.quad = NULL
#        
#    cdef void setData(self, RGBQUAD* quad):
#        self.quad = quad
#        
#    def __repr__(self):
#        if self.quad == NULL:
...
#
#cdef _FiRGBQuad FiRGBQuad(RGBQUAD* quad):
#    cdef _FiRGBQuad rgb
#    rgb = _FiRGBQuad()
#    rgb.setData(quad)
#    return rgb
...
cdef _DibWrapper DibWrapper(FIBITMAP* dib, oldimg):
...
cdef class _BitmapInfo:
    ...
    """
    cdef readonly DWORD size
    cdef readonly LONG  width 
    cdef readonly LONG  height 
    cdef readonly WORD  planes 
    cdef readonly WORD  bit_count
    cdef readonly DWORD compression 
    cdef readonly DWORD size_image 
    cdef readonly LONG  xppm 
    cdef readonly LONG  yppm 
    cdef readonly DWORD colors_used 
    cdef readonly DWORD colors_important
...
cdef _BitmapInfo BitmapInfo(BITMAPINFOHEADER *header):
...
#        self.tag = NULL
#        
#    #cdef void setData(self, FITAG* tag):
#    #    self.tag = tag
#
#cdef _FITag FITag(FITAG* tag):
#    cdef _FITag t
#    t = _FITag()
#    t.setData(tag)
#    return t
...
cdef class _MemoryIO:
    ...
    def __init__(self, Image img not None, int format=-1, int flags=0):
        ...
        if format == FIF_UNKNOWN:
            ...
            # sensible default
            if self._format == FIF_ICO:
                self._format = FIF_PNG
        else:
            self._format = <FREE_IMAGE_FORMAT>format
            
        if self._format < 0 or self._format > FreeImage_GetFIFCount():
            raise ValueError("Invalid image format")
        
        mem = FreeImage_OpenMemory(NULL, 0)
        if mem == NULL:
            raise dispatchFIError(FreeImageMemoryError, "FreeImage_OpenMemory() has failed")

        if not FreeImage_SaveToMemory(self._format, img._dib, mem, flags):
            FreeImage_CloseMemory(mem)
            mem = NULL
            raise dispatchFIError(OperationError, "Can't save image to memory")

        FreeImage_SeekMemory(mem, 0, stdio.SEEK_END)
        self.size = FreeImage_TellMemory(mem)
        FreeImage_SeekMemory(mem, 0, stdio.SEEK_SET)
        self._mem = mem
    ...
    def __dealloc__(self):
        if self._mem != NULL:
            FreeImage_CloseMemory(self._mem)
        self._mem = NULL
        self._img.buffers -= 1
    ...
    def __getreadbuffer__(self, int index, void **ptr):
        ...
        FreeImage_AcquireMemory(self._mem, <BYTE**>ptr, &size)
        return size
    ...
    def __str__(self):
        ...
        fipython.PyObject_AsReadBuffer(self, <const_void_ptr *>&buf, &size)
    ...
    def tell(self):
        ...
        """
        return FreeImage_TellMemory(self._mem)
    ...
    def seek(self, int offset, unsigned whence=stdio.SEEK_SET):
        ...
        if not FreeImage_SeekMemory(self._mem, offset, whence):
    ...
    def read(self, signed size=-1):
        ...
        # read up the EOF max
        pos = FreeImage_TellMemory(self._mem)
        ...
        # copy operation. 
        amount = FreeImage_ReadMemory(buf, <unsigned>rsize, 1, self._mem)
...
###
cdef int floodfill(FIBITMAP* dib, unsigned red, unsigned green, unsigned blue, unsigned alpha) nogil:
    ...
    if image_type == FIT_BITMAP and bpp == 24:
        ...
        for y from 0 <= y < height:
            ...
            for x from 0 <= x < width:
                pixel[FI_RGBA_BLUE] = blue
                pixel[FI_RGBA_GREEN] = green
                pixel[FI_RGBA_RED] = red
                pixel += 3
            # next line
            bits += pitch
...
cdef void removeAllMetadata(FIBITMAP *dib):
    """Remove ICC profile and metadata
    """
    FreeImage_DestroyICCProfile(dib) 
    for model in _META_MODELS.values():
        FreeImage_SetMetadata(<FREE_IMAGE_MDMODEL>model, dib, NULL, NULL)

cdef int cloneICCProfile(FIBITMAP *src, FIBITMAP *dst):
    """Clone ICC profile from src to dst
    """
    cdef FIICCPROFILE* icc
    icc = FreeImage_GetICCProfile(src)
    if icc != NULL:
        FreeImage_CreateICCProfile(dst, icc.data, icc.size)
        return 1
    else:
        FreeImage_DestroyICCProfile(dst)
        return 0
...
def _new_image(unsigned width, unsigned height, unsigned bpp, 
        ...
        cdef Image img
        with nogil:
            dib = FreeImage_Allocate(width, height, bpp, red_mask, green_mask, blue_mask)
...
cdef class Image:
    ...
    cdef from_filename(self, char* filename, int flags):
        ...
        self._format = FreeImage_GetFileType(filename, 0)
        if self._format == FIF_UNKNOWN:
            raise dispatchFIError(UnknownImageError, filename)
        with nogil:
            self._dib = FreeImage_Load(self._format, self.filename, flags)
        if self._dib == NULL:
    ...
    cdef from_buffer(self, object obj, int flags):
        ...
        fipython.PyObject_AsReadBuffer(obj, <const_void_ptr *>&buffer, &size)
        mem = FreeImage_OpenMemory(buffer, size)
        if mem == NULL:
            raise dispatchFIError(FreeImageMemoryError, "FreeImage_OpenMemory() has failed")
        self._format = FreeImage_GetFileTypeFromMemory(mem, 0)
        if self._format == FIF_UNKNOWN:
            FreeImage_CloseMemory(mem)
            mem = NULL
            raise dispatchFIError(UnknownImageError, "<buffer>")
        
        self._dib = FreeImage_LoadFromMemory(self._format, mem, flags)
        FreeImage_CloseMemory(mem)
    ...
    cdef _init_infos(self):
        self.width = FreeImage_GetWidth(self._dib)
        self.height = FreeImage_GetHeight(self._dib)
        self._icc = NULL
    ...
    property format:
        ...
        Format is the image format like FIF_JPEG or FIF_PNG.
        """
        def __get__(self):
            return self._format
    ...
    property type:
        ...
        Type referes to the data type of the image like FIT_BITMAP or FIT_RGBF.
        """
        def __get__(self):
            return FreeImage_GetImageType(self._dib)
    ...
    property mimetype:
        ...
        """
        def __get__(self):
            return FreeImage_GetFIFMimeType(self._format)
    ...
    property dpi_x:
        ...
        def __get__(self):
            self._check_closed()
            return round(FreeImage_GetDotsPerMeterX(self._dib) * INCH_METER)
        
        def __set__(self, float dpi):
            if dpi < 0:
                raise ValueError("DPI must be positive or zero.")
            self._check_closed()
            FreeImage_SetDotsPerMeterX(self._dib, int(round(dpi * METER_INCH)))
    ...
    property dpi_y:
        ...
        def __get__(self):
            self._check_closed()
            return round(FreeImage_GetDotsPerMeterY(self._dib) * INCH_METER)
        
        def __set__(self, float dpi):
            if dpi < 0:
                raise ValueError("DPI must be positive or zero.")
            self._check_closed()
            FreeImage_SetDotsPerMeterY(self._dib, int(round(dpi * METER_INCH)))
    ...
    property dpm_x:
        ...
        def __get__(self):
            self._check_closed()
            return int(FreeImage_GetDotsPerMeterX(self._dib))
        
        def __set__(self, unsigned dpm):
            FreeImage_SetDotsPerMeterX(self._dib, dpm)
    ...
    property dpm_y:
        ...
        def __get__(self):
            self._check_closed()
            return int(FreeImage_GetDotsPerMeterY(self._dib))
        
        def __set__(self, unsigned dpm):
            self._check_closed()
            FreeImage_SetDotsPerMeterY(self._dib, dpm)
    ...
    property colors_used:
        ...
        def __get__(self):
            self._check_closed()
            return FreeImage_GetColorsUsed(self._dib)    
    ...
    property bpp:
        ...
        def __get__(self):
            self._check_closed()
            return FreeImage_GetBPP(self._dib)
    ...
    property has_icc:
        ...
        def __get__(self):
            if self._icc == NULL:
                self._check_closed()
                with nogil:
                    self._icc = FreeImage_GetICCProfile(self._dib)
            if self._icc.data:
                return True
            else:
                return False
    ...
    property icc_cmyk:
        ...
        def __get__(self):
            if not self.has_icc:
                return None
            if (self._icc.flags & FIICC_COLOR_IS_CMYK) == FIICC_COLOR_IS_CMYK:
                return True
            else:
                return False
    ...
    property color_type:
        ...
        Get color type of the image, e.g. FIC_MINISBLACK, FIC_RGB, FIC_CMYK ... 
        """
        def __get__(self):
            self._check_closed()
            return FreeImage_GetColorType(self._dib)
        
    property color_type_name:
        """color_type_name -> str
        """
        def __get__(self):
            cdef FREE_IMAGE_COLOR_TYPE ct
            self._check_closed()
            ct = FreeImage_GetColorType(self._dib)
    ...
    property is_transparent:
        ...
        def __get__(self):
            self._check_closed()
            return FreeImage_IsTransparent(self._dib)
    ...
    property has_bg_color:
        ...
        def __get__(self):
            self._check_closed()
            return FreeImage_HasBackgroundColor(self._dib)
    ...
    property rgb_mask:
        ...
        def __get__(self):
            self._check_closed()
            if FreeImage_GetColorType(self._dib) != FIC_RGB:
                return None
            return (FreeImage_GetRedMask(self._dib), 
                    FreeImage_GetGreenMask(self._dib), 
                    FreeImage_GetBlueMask(self._dib))
    ...
    property has_pixels:
        """Check if image is loaded with FIF_LOAD_NOPIXELS
        
        @return: True if image has pixel and isn't loaded with FIF_LOAD_NOPIXELS
        """
        def __get__(self):
            return bool(FreeImage_HasPixels(self._dib))
    ...
    def floodfill(self, int red, int green, int blue):
        ...
        if result == ERR_UNSUP:
            raise NotImplementedError("floodfill for type: %i, bpp:  %i" %
                (FreeImage_GetImageType(self._dib), self.bpp))
    ...
    def save(self, char* filename, int format=-1, int flags=0):
        ...
        if format < 0 or format > FreeImage_GetFIFCount() - 1:
            raise ValueError("Invalid format %i" % format)
        with nogil:
            result = FreeImage_Save(<FREE_IMAGE_FORMAT>format, self._dib, filename, flags) 
        if not result:
    ...
    # **********************************************************************
    # scale
    def resize(self, int width, int height, int filter=FILTER_BOX):
        ...
        self._check_closed()
        with nogil:
            dib = FreeImage_Rescale(self._dib, width, height, <FREE_IMAGE_FILTER>filter)
        if dib == NULL:
    ...
    def paste(self, Image src not None, unsigned left, unsigned top, unsigned alpha = 256):
        ...
        with nogil:
            result = FreeImage_Paste(self._dib, src._dib, left, top, alpha)
        if not result:
    ...
    def crop(self, int left, int top, int right, int bottom):
        ...
        # XXX more checks
        with nogil:
            dib = FreeImage_Copy(self._dib, left, top, right, bottom)
        if dib == NULL:
    ...
    def convert_4bits(self):
        ...
        """
        return self.convert_helper(FreeImage_ConvertTo4Bits)

    def convert_8bits(self):
        """convert_8bits() -> new image
        """
        return self.convert_helper(FreeImage_ConvertTo8Bits)

    def greyscale(self):
        """greyscale() -> new image
        """
        return self.convert_helper(FreeImage_ConvertToGreyscale)

    def convert_16bits555(self):
        """convert_16bits555() -> new image
        """
        return self.convert_helper(FreeImage_ConvertTo16Bits555)

    def convert_16bits565(self):
        """convert_16bits565() -> new image
        """
        return self.convert_helper(FreeImage_ConvertTo16Bits565)

    def convert_24bits(self):
        """convert_24bits() -> new image
        """
        return self.convert_helper(FreeImage_ConvertTo24Bits)

    def convert_32bits(self):
        """convert_32bits() -> new image
        """
        return self.convert_helper(FreeImage_ConvertTo32Bits)

    def convert_rgbf(self):
        """convert_rgbf() -> new image
        """
        return self.convert_helper(FreeImage_ConvertToRGBF)
    ...
    def to_standard(self, bint scale_linear=True):
        ...
        self._check_closed()
        with nogil:
            dib = FreeImage_ConvertToStandardType(self._dib, scale_linear)
        if dib == NULL:
            raise dispatchFIError(OperationError, "Failed to convert image")
        return Image(_bitmap=DibWrapper(dib, self))
    ...
    def to_type(self, unsigned dst_type, bint scale_linear=True):
        ...
        self._check_closed()
        with nogil:
            dib = FreeImage_ConvertToType(self._dib, <FREE_IMAGE_TYPE>dst_type, scale_linear)
        if dib == NULL:
            raise dispatchFIError(OperationError, "Failed to convert image")
        return Image(_bitmap=DibWrapper(dib, self))
    ...
    def dither(self, unsigned dither_alg):
        ...
        self._check_closed()
        with nogil:
            dib = FreeImage_Dither(self._dib, <FREE_IMAGE_DITHER>dither_alg)
        if dib == NULL:
    ...
    def threshold(self, unsigned threshold):
        ...
        with nogil:
            dib = FreeImage_Threshold(self._dib, <BYTE>threshold)
    ...
    def flipHorizontal(self):
        ...
        copy = self.clone()
        with nogil:
            FreeImage_FlipHorizontal(copy._dib)
        return copy
    ...
    def flipVertical(self):
        ...
        copy = self.clone()
        with nogil:
            FreeImage_FlipVertical(copy._dib)
        return copy
    ...
    def rotateEx(self, double angle, double x_shift=0, double y_shift=0, 
        ...
        wrapper around FreeImage_RotateEx
        ...
        with nogil:
            dib = FreeImage_RotateEx(self._dib, angle, x_shift, y_shift, 
    ...
    #    self._icc = FreeImage_CreateICCProfile(self._dib, <char*>cp, len(profile))
    ...
    def removeICC(self):
        ...
        """
        self._icc = NULL
        FreeImage_DestroyICCProfile(self._dib)
    ...
    def getMetadataCount(self):
        ...
        for name, model in _META_MODELS.iteritems():
            counts[name] = int(FreeImage_GetMetadataCount(model, self._dib))
        return counts
    
    def getMetadata(self):
        """getMetadata() -> dict
        """
        cdef FIMETADATA *handle = NULL
        cdef FITAG *tag
        ...
        self._check_closed()
        for name, model in _META_MODELS.iteritems():
            handle = FreeImage_FindFirstMetadata(model, self._dib, &tag)
            if handle == NULL:
                continue
            meta = metas.setdefault(name, {})
            while 1:
                key = <char *>FreeImage_GetTagKey(tag)
                string = <char *>FreeImage_TagToString(model, tag, NULL)
                descr = <char *>FreeImage_GetTagDescription(tag)
                if descr != NULL:
                    meta[key] = string, descr
                else:
                    meta[key] = string, None
                if not FreeImage_FindNextMetadata(handle, &tag):
    ...
    def copyMetadataFrom(self, Image src not None):
        ...
        if src._dib == NULL:
            raise IOError("Operation on closed source image")
        if not FreeImage_CloneMetadata(self._dib, src._dib):
            raise dispatchFIError(OperationError, "Failed to copy metadata")
    
    def clearMetadata(self):
        self._check_closed()
        for name, model in _META_MODELS.iteritems():
            if not FreeImage_SetMetadata(model, self._dib, NULL, NULL):
    ...
    def getInfoHeader(self):
        ...
        """
        cdef BITMAPINFOHEADER *header
        self._check_closed()
        header = FreeImage_GetInfoHeader(self._dib)
    ...
    def adjustColors(self, float brightness=0.0, float contrast=0.0, 
        ...
        if not FreeImage_AdjustColors(self._dib, brightness, contrast,
                                      gamma, invert):
    ...
    def getHistogram(self, int channel=FICC_BLACK):
        """getHistogramm([channel=FICC_BLACK]) -> list[256]
        """
        cdef int bpp, i
        cdef DWORD hist[256]
        
        self._check_closed()
        bpp = FreeImage_GetBPP(self._dib)
        
        if bpp == 24 or bpp == 32:
            if not (channel == FICC_BLACK or channel == FICC_RED or 
                    channel == FICC_GREEN or channel == FICC_BLUE):
                raise ValueError("%i bpp image supports histogram for red, "
                                 "green, blue or black channels only, got %i."
                                 % (bpp, channel))
        elif bpp == 8:
            if channel != FICC_BLACK:
            ...
            raise ValueError("%i bpp is not supported by histogram function."
                             % bpp)
        if not FreeImage_GetHistogram(self._dib, hist,
    ...
    def hline(self, unsigned y, unsigned xstart, signed xend, unsigned linewidth=1,
        ...
        self._check_closed()
        image_type = FreeImage_GetImageType(self._dib)
        width = FreeImage_GetWidth(self._dib)
        height = FreeImage_GetHeight(self._dib)
        bpp = FreeImage_GetBPP(self._dib)
        ...
        #elif image_type == FIT_BITMAP and bpp == 1:
        #    # bitonal bitmap
        #    self.hline_1bpp(red, linewidth, ys, xstart, xend)
        elif image_type == FIT_BITMAP and bpp == 8:
    ...
    cdef int hline_rgb24(self, RGBQUAD *color, unsigned linewidth,
        ...
        cdef BYTE *pixel
        cdef unsigned x, y
        cdef FIBITMAP* dib = self._dib

        with nogil:
            for y from y0 <= y < y0+linewidth:
                pixel = FreeImage_GetScanLine(dib, y)
                for x from 0 <= x < xe - xs:
                        pixel[FI_RGBA_BLUE] = color.rgbBlue
                        pixel[FI_RGBA_GREEN] = color.rgbGreen
                        pixel[FI_RGBA_RED] = color.rgbRed
                        pixel += 3
        return 0

    cdef int hline_mono(self, BYTE color, unsigned linewidth,
                   unsigned y0, unsigned xs, unsigned xe):
...
#    cdef int hline_1bpp(self, BYTE color, unsigned linewidth,
#                   unsigned y0, unsigned xs, unsigned xe):
...
#        return 0
    ...
    cdef int vline_rgb24(self, RGBQUAD *color, unsigned linewidth,
    ...
    cdef int vline_mono(self, BYTE color, unsigned linewidth,
                   unsigned x0, unsigned ys, unsigned ye):
...
#    cdef int vline_1bpp(self, BYTE color, unsigned linewidth,
#                   unsigned x0, unsigned ys, unsigned ye):
...
def FormatInfo_from_mimetype(cls, char *mime):
    ...
    """
    format = FreeImage_GetFIFFromMime(mime)
    return cls(format)
...
cdef class FormatInfo:
    ...
    property name:
        def __get__(self):
            return FreeImage_GetFormatFromFIF(self._format)
    
    property description:
        def __get__(self):
            return FreeImage_GetFIFDescription(self._format)
    
    property magic_reg_expr:
        def __get__(self):
            return FreeImage_GetFIFRegExpr(self._format)

    property supports_reading:
        def __get__(self):
            return FreeImage_FIFSupportsReading(self._format)

    property supports_writing:
        def __get__(self):
            return FreeImage_FIFSupportsWriting(self._format)
        
    property supports_icc:
        def __get__(self):
            return FreeImage_FIFSupportsICCProfiles(self._format)
            
    property supports_nopixels:
        def __get__(self):
            return FreeImage_FIFSupportsNoPixels(self._format)

    def getExtensions(self):
        exts = FreeImage_GetFIFExtensionList(self._format)
        return exts.split(',')
    
    def getSupportsExportType(self, int type):
        if type < 0 or type > FIT_RGBAF:
            raise ValueError("Invalid type %i" % type)
        return FreeImage_FIFSupportsExportType(self._format, 
                                               <FREE_IMAGE_TYPE>type)            
        
    def getSupportsExportBPP(self, int bpp):
        return FreeImage_FIFSupportsExportBPP(self._format, bpp)            
...
def getCompiledFor():
    ...
    """
    return (FREEIMAGE_MAJOR_VERSION, FREEIMAGE_MINOR_VERSION,
...
def getFormatCount():
    """getFormatCount() -> int
    """
    return FreeImage_GetFIFCount()

def lookupX11Color(char *name):
    """lookupX11Color(name) -> (r, g, b)
    """
    cdef BYTE red, green, blue
    if not FreeImage_LookupX11Color(name, &red, &green, &blue):
...
def lookupSVGColor(char *name):
    ...
    """
    cdef BYTE red, green, blue
    if not FreeImage_LookupSVGColor(name, &red, &green, &blue):
...
def jpegTransform(char *src, char *dst, unsigned int op, unsigned int perfect=0):
    ...
    if op > FIJPEG_OP_ROTATE_270:
        raise ValueError("OP not in range 0-7")
    
    with nogil:
        result = FreeImage_JPEGTransform(src, dst, 
                                         <FREE_IMAGE_JPEG_OPERATION>op, perfect) 
    if not result:
