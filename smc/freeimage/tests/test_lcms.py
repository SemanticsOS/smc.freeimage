...
class TestLCMS(TestImageBase):
    ...
    def test_lcmsIccCache(self):
        ...
        icc = self.tiff.getICC()
        cache.lookupByImage(self.tiff, "sRGB", lcms.TYPE_BGR_8, lcms.INTENT_PERCEPTUAL, 0)
        self.assertEqual(cache.creations, 1)
        self.assertEqual(list(cache), [(icc, "sRGB", lcms.TYPE_BGR_8, lcms.INTENT_PERCEPTUAL, 0)])

        cache.lookupByImage(self.tiff, "sRGB", lcms.TYPE_BGR_8, lcms.INTENT_PERCEPTUAL, 0)
        self.assertEqual(cache.creations, 1)
        cache.lookup(icc, "sRGB", lcms.TYPE_BGR_8, lcms.INTENT_PERCEPTUAL, 0)
        self.assertEqual(cache.creations, 1)
        ...
        entry = (icc, "sRGB", lcms.TYPE_BGR_8, lcms.INTENT_PERCEPTUAL, 0)
        ...
        #    cache.applyTransform(self.tiff)
        #    print time() - start
        cache.lookup("gray", "gray", lcms.TYPE_GRAY_8, lcms.INTENT_PERCEPTUAL, 0)
        cache.lookup("sRGB", "sRGB", lcms.TYPE_ABGR_8, lcms.INTENT_PERCEPTUAL, 0)
        cache.lookup("sRGB", "sRGB", lcms.TYPE_BGR_16, lcms.INTENT_PERCEPTUAL, 0)
